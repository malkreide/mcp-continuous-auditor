...
- **Release gap** — the identity probe asks whether the reported version is *correct*; `scripts/release_gap.py` asks whether it is *current*. A repository can be green, audited and entirely fixed while every `pip install` still hands out the broken release, because CI tests the branch and not the artifact. `meteoswiss-mcp` shipped an import error to every fresh install for three days with `main` already corrected, and it took an outside bug report to notice. The probe compares the published PyPI version, the release tags and the unreleased commits, weighs a `fix:` differently from a `docs:`, and reports an unreachable index as `UNKNOWN` rather than as success. It reads the **Simple API** — the surface `pip` installs from — because PyPI's JSON API was measured lagging it by minutes on both the latest version and the `yanked` flag; the JSON API is kept as a fallback and a second opinion, and where the two disagree the answer is `UNCONFIRMED` rather than guessed. That also closes a blind spot the version number alone cannot see: a *yanked* release looks identical to a healthy one — the version exists, the tag matches, CI is green — while every `pip install` quietly resolves to something older.
- **Published probe** — the identity probe reads a repository and the release gap compares version *numbers*; neither opens the artifact. `swiss-efv-mcp` passes both (PyPI 0.3.0, `main` 0.3.0, `src/` clean) while the package every user installs sends `Mozilla/5.0 … Chrome/124.0` — it impersonates a browser to upstreams. `scripts/published_probe.py` installs the distribution into a throwaway venv and reads the User-Agent the shipped code actually puts on the wire: 16 of 33 portfolio packages announced a version they were not, with the fix merged in every one. Where it cannot resolve a value it reports `UNVERIFIED`, never clean — an earlier version that conflated "found nothing" with "there is nothing" called 24 packages unremarkable, 16 of which were drifting.
